...
import com.alessandro_molinaro.social_network.repository.AccountUtenteRepository;
...
public class AccountingService {
  ...
  @Autowired AccountUtenteRepository accountUtenteRepository;
  ...
  public Utente verificaRegistrazione(AccountUtente a)
    ...
    if (ol.isPresent()) {
      ...
      if (!l.getPassword().equals(a.getPassword())) throw new PasswordErrataException();
      return l.getDatiUtente();
    } else throw new UsernameErratoException();
  }
  ...
  public Utente registraUtente(Utente u)
      throws EmailEsistenteException, UtenteNonEsistenteException {
    if (utenteRepository.existsByEmail(u.getEmail())) throw new EmailEsistenteException();
  ...
  public AreaGeografica aggiungiModificaAreaGeografica(Long userId, AreaGeografica ag)
    ...
    AreaGeografica area =
        areaGeograficaRepository.findByNazioneAndRegioneAndCitta(
            ag.getNazione(), ag.getRegione(), ag.getCitta());
    ...
    if (utente.getAreaGeografica() != null) {
      Optional<AreaGeografica> oa =
          areaGeograficaRepository.findById(utente.getAreaGeografica().getId());
      if (oa.isPresent()) {
