...
public class PostController {
    ...
    public ResponseEntity<List<Post>> postDiAmici(@PathVariable Long userId, @PathVariable int numPage, @PathVariable int numInPage){
        try {
            //return postService.getPostDegliAmici(userId, numPage, numOfPage);
            return new ResponseEntity(postService.getPostDegliAmici(userId, numPage-1, numInPage), HttpStatus.OK);
        } catch (UtenteNonEsistenteException e) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "l'utente non esiste", e);
            //return new ResponseEntity(null, HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity postListUtente(@PathVariable Long userId, @PathVariable int numPag, @PathVariable int numInPage){
        try{
