...
public class AccountingController {
  ...
  public ResponseEntity registrazione(
    ...
    try {
      ...
      // return new ResponseEntity(new Messaggio("email già associata ad un utente
      // registrato"),HttpStatus.BAD_REQUEST);
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "email già associata ad un utente registrato", e);
    } catch (UtenteNonEsistenteException e) {
      // return new ResponseEntity(new Messaggio("utente non ancora registrato per poter aggiungere
      // un indirizzo"),HttpStatus.BAD_REQUEST);
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "registrazione non andata a buon fine!!", e);
    } catch (UsernameEsistenteException e) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "username già associata ad un utente registrato", e);
    }
  }

  @PostMapping(value = "/{userId}/set_geographic_area")
  public ResponseEntity setAreaGeografica(
      @PathVariable Long userId, @RequestBody AreaGeografica a) {
    try {
  ...
  public ResponseEntity verificaRegistrazione(@Valid @RequestBody AccountUtente accountUtente) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "l'utente non esiste oppure è errato", e);
    }
  }
}
